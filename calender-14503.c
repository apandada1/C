...
int daysgoneby(int y) /* Counts the no of days elapsed since 1/1/1800 to the end of the previous year */
{
	int a;
	int b;
	a=y-1800;
...
int cal(int m, int y) /* Prints the calender of the month */
{
	...
	(2+q)%7 is used instead of (2+q), so that if q=5,6, or 7, the first line of calender will be
	just under Su Mo Tu ... */
	{
		printf("   ");
		j=j+1;
	...
	while(i<=daysinmonth(m,y))
	{
		...
		printf("%d ",i);
		i=i+1;
		if((q+2+i)%7==1)/* Goes to the next line after Saturday*/
		{
			printf("\n");
		}
	}
